...
export class GameCollection {
   ...
   */
  public addGame(game: Videogame, callback: Callback) {
    const filePath: string = this.getFilePath(game.id);
    if (fs.existsSync(filePath)) {
      callback(false, chalk.red(`Videogame already exists at ${this.user} collection!`));
      return;
    }
   ...
   */
  public updateGame(game: Videogame, callback: Callback) {
    const filePath: string = this.getFilePath(game.id);
    if (!fs.existsSync(filePath)) {
    ...
    fs.writeFile(filePath, JSON.stringify(game, null, 2), () => {
      callback(true, chalk.green(`Videogame updated at ${this.user} collection!`));
    });
  }
   ...
   */
  public removeGame(id: number, callback: Callback) {
    ...
    fs.rm(filePath, () => {
      callback(true, chalk.green(`Videogame removed from ${this.user} collection!`));
    });
  }
   ...
   */
  public readGame(id: number, callback: Callback) {
    const filePath: string = this.getFilePath(id);
